...
export class PlanIshraneComponent implements OnInit {
  ...
  prikaziMacrosObroka(i: number, j: number) {
    if (this.detaljiObrok[0] == i && this.detaljiObrok[1] == j)
      this.detaljiObrok = [-1, -1];
    else this.detaljiObrok = [i, j];
  }

  prikaziMacrosHrane(i: number, j: number, z: number) {
    if (
      this.detaljiHrana[0] == i &&
      this.detaljiHrana[1] == j &&
      this.detaljiHrana[2] == z
    )
      this.detaljiHrana = [-1, -1, -1];
    else this.detaljiHrana = [i, j, z];
  }

  izracunajKalorijeHrana(hrana: Hrana, kolicina: number): string {
    return (
      ((hrana.masti * 9 + hrana.proteini * 4 + hrana.ugljeniHidrati * 4) *
        kolicina) /
      100
    ).toFixed(1);
  }
  ...
  dodajKomentar() {
    let noviKomentar: Odgovor = {
      koJeOdgovorio: this.loginService.logovaniUsername,
      text: this.komentarControl.value
    };
    console.log(noviKomentar);
    this.plan.komentari.push(noviKomentar);
    this.planIshraneService.addKomentar(
      this.parametri.uK,
      this.parametri.uI,
      this.parametri.dat,
      this.parametri.naz,
