...
export class DodajPlanIshraneComponent implements OnInit {
  ...
  trenutniObrok:Obrok={deloviObroka:[],ukupnaVlakna:0,ukupneKalorije:0,ukupneMasti:0,ukupniProteini:0,ukupniUgljeniHidrati:0,vreme:""};
  trenutnaHrana:Hrana=null;
  nizDana:Dan[]=[];
  trenutniDan:Dan={obroci:[],vlakna:0,kalorije:0,masti:0,proteini:0,ugljeniHidrati:0};
  ...
  constructor(private route:ActivatedRoute,private router:Router,private location:Location,private loginService:LoginService,private hranaService:HranaService,private planIshraneService : PlanIshraneService) { }
  ...
  dodajObrok(){
    ...
    this.trenutniObrok = {deloviObroka:[],ukupnaVlakna:0,ukupneKalorije:0,ukupneMasti:0,ukupniProteini:0,ukupniUgljeniHidrati:0,vreme:""};
    }
  }
  ...
  dodajDan(){
    ...
    this.trenutniDan = {obroci:[],vlakna:0,kalorije:0,masti:0,proteini:0,ugljeniHidrati:0};
  }

  posaljiPlan(){
    let noviPlan : PlanIshrane = {dani:this.nizDana,usernameKlijenta:this.usernameKlijenta,usernameInstruktora:this.usernameInstruktora}
  ...
  dodajUObrok(){
    if(this.kolicinaControl.value!=""){
      let noviDeoObroka:DeoObroka = {hrana:this.trenutnaHrana,kolicina:this.kolicinaControl.value};
  ...
  updatujKalorijeUObroku(deoObroka:DeoObroka,flag:number){
    let skaliraniProtein:number = deoObroka.hrana.proteini*deoObroka.kolicina/100;
    let skaliranaMast:number = deoObroka.hrana.masti*deoObroka.kolicina/100;
    let skaliranaVlakna:number = deoObroka.hrana.vlakna*deoObroka.kolicina/100;
    let skaliraniHidrati:number = deoObroka.hrana.ugljeniHidrati*deoObroka.kolicina/100;
    ...
    this.trenutniObrok.ukupneKalorije+=skaliraniProtein*4 + skaliranaMast*9 + skaliraniHidrati*4*flag;
    this.trenutniObrok.ukupneKalorije=+this.trenutniObrok.ukupneKalorije.toFixed(1);
  }
  ...
  prikaziDetaljeObrok(index:number){
    console.log(index);
    if(index == this.detaljiZaObrok)
      this.detaljiZaObrok = -1;
    else
      this.detaljiZaObrok = index;
  }

  prikaziDetaljeDan(index:number){
    console.log(index);
    if(index == this.detaljiZaDan)
      this.detaljiZaDan = -1;
    else
      this.detaljiZaDan = index;
  }
